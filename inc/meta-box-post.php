...
function mega_add_box_post() {
	global $meta_box_post_video, $meta_box_post_gallery;
...
$meta_box_post = array(
	'id' => 'mega-meta-box-post',
	'title' =>  __( 'Featured On Frontpage Settings', 'mega' ),
	'page' => 'post',
	'context' => 'normal',
	'priority' => 'high',
	'fields' => array(
		array(
    	   'name' => __( 'Frontpage Slider', 'mega' ),
    	   'desc' => __( "Used in Frontpage Slider", 'mega' ),
    	...
    	array(
    	   'name' => __( 'Frontpage Far Right Static', 'mega' ),
    	   'desc' => __( "Used in Frontpage Far Right Static", 'mega' ),
    	   'id' => $prefix.'post_static',
    	   'type' => "select",
		   'std' => __( 'No', 'mega' ),
		   'options' => array( __( 'No', 'mega' ),__( 'Yes', 'mega' ) )
    	),
    	array(
    	   'name' => __( 'Frontpage Beneath Left', 'mega' ),
    	   'desc' => __( "Used in Frontpage Beneath Left", 'mega' ),
    	   'id' => $prefix.'post_left',
    	   'type' => "select",
		   'std' => __( 'No', 'mega' ),
		   'options' => array(__( 'No', 'mega' ),__( 'Yes', 'mega' ) )
    	...
    	array(
    	   'name' => __( 'Frontpage Beneath Right', 'mega' ),
    	   'desc' => __( "Used in Frontpage Beneath Right", 'mega' ),
    	   'id' => $prefix.'post_right',
    	   'type' => "select",
		   'std' => __( 'No', 'mega' ),
		   'options' => array(__( 'No', 'mega' ),__( 'Yes', 'mega' ) )
    	)
 ...
 * Add metabox .
 */
add_action( 'admin_menu', 'mega_add_boxes_post' );
 
function mega_add_boxes_post() {
	global $meta_box_post;
	add_meta_box($meta_box_post['id'], $meta_box_post['title'], 'mega_show_box_post', $meta_box_post['page'], $meta_box_post['context'], $meta_box_post['priority']);
}
/**
 * Callback function to show fields in meta box.
 */
function mega_show_box_post() {
...
function mega_save_data_post($post_id) {
	...
	if ( isset($_POST['mega_meta_box_nonce'])) {
		...
		foreach ($meta_box_post['fields'] as $field) {
			...
			$new = $_POST[$field['id']];
			if ($new && $new != $old) {
				if($field['id']=='mega_post_static'||$field['id']=='mega_post_left'||$field['id']=='mega_post_right'){
					$args = array( 'posts_per_page' => 1000000, 'post_status' =>'any','post_type' =>'post', );
					$myposts = get_posts( $args );
					foreach ( $myposts as $post_check ) {
						delete_post_meta($post_check->ID, $field['id'],$new);
					}
					update_post_meta($post_id, $field['id'], stripslashes(htmlspecialchars($new)));
				}
				else{
				...
				if($field['id']=='mega_post_static'||$field['id']=='mega_post_left'||$field['id']=='mega_post_right'){
					...
					$myposts = get_posts( $args );
					foreach ( $myposts as $post_check ) {
						$post_status=get_post_meta( $post_check->ID,$field['id']);
						if($post_status){
							delete_post_meta($post_check->ID, $field['id']);
						}
					}
				}
				else{
